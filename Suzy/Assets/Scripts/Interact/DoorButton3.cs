...
public class DoorButton3 : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetMouseButtonDown (0)) {
			...
			if (Camera.GetComponent<PlayerRaycasting> ().whatIHit.collider.name == "Button3"
				&& Camera.GetComponent<PlayerRaycasting> ().whatIHit.distance <= interactionDistance) {
				//Button Logic
				if (!pushed) {
					DoorKeypad.GetComponent<DoorPasswordScript> ().enterNumber (3);
